...
# how much area?
st_area(tst) %>% sum() %>% measurements::conv_unit(., from = "m2", to="hectare") # hectares

st_area(tst) %>% sum() %>% measurements::conv_unit(., from = "m2", to="km2") # sq km
...
# mdw map
mapview(tst, map.types=mapbases, col.regions="seagreen2", color="seagreen", lwd=2, alpha.regions=0.1, legend=TRUE, layer.name="Meadows") +
